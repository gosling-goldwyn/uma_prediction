...
def train_model(model_type, X, y, target_mode, horse_info="included", **kwargs):
    print(
...
if __name__ == "__main__":
    ...
    for target_mode in ["default", "top3"]:
        # With horse info
        X_rf, y_rf = preprocess_data(
            df.copy(), model_type="rf", target_mode=target_mode
        )
        train_model("rf", X_rf, y_rf, target_mode)
        X_lgbm, y_lgbm, cats_lgbm = preprocess_data(
            df.copy(), model_type="lgbm", target_mode=target_mode
        )
        train_model("lgbm", X_lgbm, y_lgbm, target_mode, categorical_features=cats_lgbm)

        # WITHOUT horse info
        X_rf_no, y_rf_no = preprocess_data(
            df.copy(), model_type="rf", target_mode=target_mode, exclude_horse_info=True
        )
        train_model("rf", X_rf_no, y_rf_no, target_mode, horse_info="excluded")
        X_lgbm_no, y_lgbm_no, cats_lgbm_no = preprocess_data(
            df.copy(),
            model_type="lgbm",
        ...
        train_model(
            ...
            target_mode,
            horse_info="excluded",
            categorical_features=cats_lgbm_no,
        )
